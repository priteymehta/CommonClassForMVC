...
namespace Pritey.Framework.Common
{
    ...
    public class TTCommonEntity 
    {
        ...
        [TTAttributs("TableName", FieldName = "UpdatedDateTime", ParamaterDataType = SqlDbType.NVarChar, isInsertField = false, isUpdateField = false, isTableField = false, isSelectField = false)]
        [Description("This Property has table name which is belong to entity")]
        public String UpdatedDateTimeFormat
        {
            get
            {
                if (this.UpdatedBy.ToString() == "00000000-0000-0000-0000-000000000000")
                {
                    return string.Empty;
                }
                else
                {
                    return this.UpdatedDateTime.GetFormatedDateTime();
                }
            }
        }
